...
export class OrganizationsController {
  ...
  @ApiOperation({ summary: 'Create a new organization (creator becomes OWNER, onboarding complete)' })
  ...
  @ApiOperation({ summary: 'List all organizations the current user is a member of' })
  @ApiResponse({ status: 200, description: 'List of user\'s organizations with their role' })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Accept an organization invitation using the token from the invite email' })
  @ApiResponse({ status: 200, description: 'Joined organization', schema: { properties: { orgSlug: { type: 'string' }, role: { type: 'string' } } } })
  ...
  @ApiOperation({ summary: 'Update organization name or logo (ADMIN or OWNER only)' })
  ...
  @OrgRoles(OrgRole.ADMIN)
  @ApiParam({ name: 'orgId', description: 'Organization ID' })
  @ApiOperation({ summary: 'Invite a user to the organization (ADMIN or OWNER only)' })
  ...
  @OrgRoles(OrgRole.ADMIN)
  @ApiParam({ name: 'orgId', description: 'Organization ID' })
  @ApiParam({ name: 'memberId', description: 'OrgMember record ID (not userId)' })
  @ApiOperation({ summary: 'Remove a member from the organization (ADMIN or OWNER only)' })
  @ApiResponse({ status: 200, description: 'Member removed' })
  @ApiResponse({ status: 403, description: 'Cannot remove owner or insufficient role' })
  removeMember(
    @Param('orgId') orgId: string,
