...
export class AuthService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.dummyHash = await argon2.hash(
      "__dummy_timing_prevention_placeholder__",
    );
  }
  ...
  async register(dto: RegisterDto): Promise<{ message: string }> {
    ...
    if (existing) {
      ...
      this.logger.info(
        { email: dto.email },
        "Register: email already exists — silently suppressed",
      );
    ...
    this.logger.info(
      ...
      "New user registered and auto-verified",
    );
  ...
  async refresh(req: Request, res: Response): Promise<{ accessToken: string }> {
    ...
    if (!tokenRecord) {
      ...
      this.logger.warn(
        { familyId },
        "Refresh token reuse detected — invalidating entire family",
      );
    ...
    try {
      ...
      if (err?.code === "P2025") {
        ...
        this.logger.warn(
          { familyId },
          "Concurrent refresh token replay detected — swallowing race condition",
        );
